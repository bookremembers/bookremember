/**
 * Created by naxiaoguang on 2016/10/17.
 */
(function() {
    // helper functions
    ...
    function factory($) {
        ...
        Crop.prototype = {
                ...
                if (typeof $.fn.hammer === 'function' || typeof Hammer !== 'undefined') {
                    ...
                    hammerit.on('touch', function(e) {
                        ...
                        timer = setInterval(function(){
                            clock++ ;
                            if(clock == 3){
                                self.$image.trigger(long_touch,[e.gesture.touches[0].pageX,e.gesture.touches[0].pageY,self.$image]);
                            }
                        },200);
                        ...
                        if(clock >= 3){
                            self.$image.trigger('move',[e.gesture.touches[0].pageX,e.gesture.touches[0].pageY,self.$image.get(0)]);
                        }else{
                        ...
                        self.$image.trigger('release', [e.gesture.touches[0].pageX,e.gesture.touches[0].pageY,self.$image.get(0)]);
                    ...
                    this.$image.on('mousedown.' + pluginName, function(e1) {
                        ...
                        var timer = setInterval(function(){
                            ...
                            if(clock == 3){
                                if(this.$image)
                                    this.$image.trigger(long_touch, [this]);
                            }
                        },500);
            ...
            updateOptions: function () {
                var self = this;
                self.img_top = self.options.img_top;
                self.img_left = self.options.img_left;
                self.img_width = self.options.img_width ;

                self.$image.css({width: self.img_width , left: self.img_left , top: self.img_top });
                ...
                if(self.$image.attr('loaded') == 'loaded'){

                    self.width = parseFloat(self.$image.attr('img_width'));
                    self.height = parseFloat(self.$image.attr('img_height'));

                    // self.minPercent = parseFloat(self.$image.attr('minPercent'));
                    self.percent = undefined ;//parseFloat(self.$image.attr('percent'));
                    ...
                    img.onload = function () {
                        ...
                        self.$image.attr('loaded','loaded');
                        self.$image.attr('img_width',self.width);
                        self.$image.attr('img_height',self.height);
                    };
                }
